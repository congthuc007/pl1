...
cancelButtons.forEach(button => {
    button.addEventListener('click', function (event) {
        ...
        if (confirmCancel) {
            // Xử lý hủy đơn hàng (hiển thị thông báo hoặc gọi API)
            alert("Đơn hàng đã bị hủy!");
            // Tại đây bạn có thể thực hiện thêm các thao tác như gọi API hoặc cập nhật trạng thái đơn hàng
        } else {
            // Ngăn chặn hành động khác nếu cần thiết
            event.preventDefault(); // Ngăn chặn hành động khác nếu có
        }
    });
});
